...
public class IndexTest {
	...
	public void test() {
		...
		recordInfo1.setContent("第一条测试数据，天天开心！");
		...
		recordInfo2.setContent("第二条测试数据，今天天气不错！");
		...
		weibos.add(recordInfo1);
		weibos.add(recordInfo2);

		TransportClient client = BuildTestClient.buildClient();
		String indexname = GetIndex.getIndex();
		String typename = GetIndex.getType();
		Index index = new Index(client, indexname, typename);
		int actualReturn = 0;
		try {
			index.createIndex(weibos);
		} catch (Exception e) {
			actualReturn = -1;
		}
