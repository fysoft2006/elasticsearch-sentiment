package zx.soft.es.core.delete;
...
public class Delete {
	...
	public Delete(TransportClient client, String index) {
		this.client = client;
		this.index = index;
	}
	...
	public DeleteByQueryResponse deleteByQuery(SearchParameters searchParameters) throws IOException {
		...
		if (searchParameters.getQ() != "*") {
			...
			response = client.prepareDeleteByQuery(index).setQuery(query).execute().actionGet();
		}
		return response;
	}
